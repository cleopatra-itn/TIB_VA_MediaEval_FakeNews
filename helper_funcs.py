...
def get_word_sent_embedding(tweet, model, tokenizer, device):
    ...
    with torch.no_grad():
        try:
            ...
            last_out, encoded_layers = model(input_ids)
    ...
    if pooled_out == None:
        return sent_word_catavg.tolist(), sent_word_sumavg.tolist(), sent_emb_2_last.tolist(), sent_emb_last.tolist(), np.array([]).tolist()
    else:
        return sent_word_catavg.tolist(), sent_word_sumavg.tolist(), sent_emb_2_last.tolist(), sent_emb_last.tolist(), pooled_out[0].cpu().numpy().tolist()
